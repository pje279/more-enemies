...
function spawn_service.do_nth_tick(event, more_enemies_data)
  ...
  --   Log.info(k)
    ...
    -- if (not spawn_service.attack_group[planet.string_val]) then
    --   spawn_service.attack_group[planet.string_val] = {
    --     unit_group = nil,
    --     tick = 0,
    --     radius = 1,
    --     chunks = {},
    ...
    -- if (spawn_service.attack_group and spawn_service.attack_group[planet.string_val] and spawn_service.attack_group[planet.string_val].tick and tick >= spawn_service.attack_group[planet.string_val].tick) then
    --   local surface = game.surfaces[planet.string_val]
    --   if (surface and surface.valid) then

    --     local attack_group = spawn_service.attack_group[planet.string_val]
    ...
    --       local function get_enemy(radius, depth)
    ...
    --           return get_enemy(1.1 * radius + 1, depth + 1)
    --         end
    ...
    --           local function get_target_entity(radius)
    --             if (not radius or radius == nil) then radius = 1 end

    --             local targets = unit_group.surface.find_entities_filtered({
    --               position = unit_group.position,
    --               radius = 32 * radius, -- TODO: Make this configurable
    --               limit = 1,
    --               force = "player",
    --             })

    --             if (not targets or not targets[1]) then return get_target_entity(1.1 * radius + 1) end

    --             return targets[1]
    --           end

    --           local target_entity = get_target_entity()
    ...
    --           attack_group.tick = tick + delay - 2 * attack_group.radius
