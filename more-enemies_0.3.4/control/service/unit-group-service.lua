...
function unit_group_service.unit_group_finished_gathering(event)
  ...
  -- Log.warn(group.unique_id)
  -- Log.warn(storage.more_enemies.groups)
  -- Log.warn(storage.more_enemies.groups[group.surface.name])
  ...
  if (selected_difficulty.string_val == "Vanilla" or selected_difficulty.value == 1) then
    ...
    -- Log.warn("Difficulty is vanilla; no need to process")
  end

  Log.warn("vanilla: " .. serpent.block(vanilla))
